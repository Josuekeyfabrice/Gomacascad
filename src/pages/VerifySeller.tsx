  ...
  const handlePawaPay = async () => {
    ...
    try {
      ...
      const pawaRes = await pawapayService.initiatePayment(
        ...
        phoneNumber,
        `Vendeur Vérifié - ${selectedPlan.name} - ${user.id}`
      );

      // 2. Track in our database (if transactions table exists)
      const { error: txError } = await supabase
        .from('transactions')
        .insert({
          buyer_id: user.id,
          seller_id: 'GOMACASCADE_ADMIN', // Target admin for platform fees
          amount: parseFloat(selectedPlan.price),
          product_id: 'VERIFICATION_FEE',
          status: 'pending',
      ...
      toast({
        ...
        description: "Vérifiez votre téléphone pour confirmer la transaction PawaPay.",
      });
      ...
      toast({
        ...
        description: error.message || "Impossible d'initier le paiement PawaPay.",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
                ...
                <div className="space-y-4">
                  <div className="relative">
                    ...
                    <div className="relative flex justify-center text-xs uppercase">
                      <span className="bg-background px-2 text-muted-foreground">Ou payer en ligne avec PawaPay</span>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="phone_pawa" className="text-xs font-bold">Numéro de téléphone PawaPay</Label>
                    <Input
                      id="phone_pawa"
                      type="tel"
                  ...
                  >
                    {isLoading ? <Loader2 className="animate-spin h-5 w-5" /> : <CreditCard className="w-5 h-5" />}
                    {isLoading ? "Initiation..." : `Payer ${selectedPlan?.price}$ via PawaPay`}
                  </Button>
